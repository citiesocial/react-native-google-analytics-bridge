...
public class GoogleAnalyticsBridge extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void trackEnhanceECommerceEvent(String trackerId, ReadableMap product, String actionListSource, String actionListName, String eventCategory, String eventAction){
        Tracker tracker = getTracker(trackerId);

        if (tracker != null) {
            if (eventAction.equals("browse")) {
                HitBuilders.EventBuilder hit = new HitBuilders.EventBuilder()
                        .addImpression(this.getPurchaseProduct(product), actionListName);

                tracker.send(hit.build());
            } else {
                ProductAction productAction = new ProductAction(eventAction)
                        .setProductListSource(actionListSource)
                        .setProductActionList(actionListName);
    ...
    private Product getPurchaseProduct(ReadableMap product) {
        ...
        if (product.hasKey("customDimensions")) {
            ReadableMap dimension = product.getMap("customDimensions");
            ReadableMapKeySetIterator iterator = dimension.keySetIterator();
            while (iterator.hasNextKey()) {
                String dimensionIndex = iterator.nextKey();
                String dimensionValue = dimension.getString(dimensionIndex);
