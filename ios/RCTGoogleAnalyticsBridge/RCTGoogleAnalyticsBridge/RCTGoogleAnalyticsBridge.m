...
RCT_EXPORT_METHOD(trackEnhanceECommerceEvent:(NSString *)trackerId product:(NSDictionary *)product actionListSource:(NSString *)actionListSource actionListName:(NSString *)actionListName eventCategory:(NSString *)eventCategory eventAction:(NSString *)eventAction)
{
    id<GAITracker> tracker = [[GAI sharedInstance] trackerWithTrackingId:trackerId];
    NSString *productId = [RCTConvert NSString:product[@"id"]];
    NSString *productName = [RCTConvert NSString:product[@"name"]];
    ...
    if ([product objectForKey:@"customDimensions"]) {
        NSDictionary *customDimensions = [RCTConvert NSDictionary:product[@"customDimensions"]];
        for (NSString *dimensionIndex in customDimensions) {
            [ecommerceProduct setCustomDimension:[dimensionIndex intValue] value:[customDimensions objectForKey:dimensionIndex]];
        }
    }
    ...
    if ([eventAction isEqualToString:@"browse"]) {
        [builder addProductImpression:ecommerceProduct
                 impressionList:actionListName
                 impressionSource:actionListSource];
    } else {
        GAIEcommerceProductAction *action = [[GAIEcommerceProductAction alloc] init];
        [action setAction:eventAction];
        [action setProductActionList:actionListName];
        [action setProductListSource:actionListSource];
